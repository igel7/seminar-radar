...
- **API従量課金なし**: Claude Code Routines / Codex Cloud いずれも各サービスの
  定額課金の利用枠内で動く。実行手順は `AGENTS.md` に共通化してあるので、
  トークン上限の都合に応じて実行エンジンを使い分けられる
  (ただし**無人の定期実行はClaudeのみ**。Codexは手動実行専用 → §3-b)
...
3. リポジトリ画面の「uploading an existing file」(Add file → Upload files)に、
   このフォルダの**中身**(README.md, AGENTS.md, sources.yaml, scripts, data, docs,
...
#### 3-b. Codex Cloud の場合(手動実行のみ)
**Codex は無人の定期実行に使えない。** エージェントのコンテナから GitHub へ
push できないため(検証記録は `CODEX_NOTES.md`)、成果物の反映に人の操作が必要になる。
巡回・抽出・データ生成そのものは正常に動くので、手動実行の受け皿としては使える。

1. Codex にこのリポジトリへのアクセスを与える
2. 手動実行のプロンプトは 3-a と同じ1行:
   `リポジトリ直下の AGENTS.md を読み、その指示に従って本日の更新を実行せよ。`
3. Codex はローカルコミットまでを行い、「push できていない」旨を報告して終了する
4. **人が Codex のタスク画面から push / PR 作成を行う。** `codex/` 接頭辞のブランチが
   GitHub に現れれば、`.github/workflows/automerge.yml` が発火し、`data/` のみを
   取り込んで main 側の `scripts/` で `docs/` を再生成する
   (このワークフローが Codex の push で実際に発火するかは未検証。初回に Actions タブで
   確認し、結果を `CODEX_NOTES.md` に追記する)

#### 3-c. 実行エンジンを切り替えるとき
定期実行は 3-a(Claude)のみで行う。Codex を使う月は、Claude のスケジュールを止めて
Codex で手動実行する運用になる(反映のたびに 3-b の4番の操作が必要)。
どちらが実行したかはコミットメッセージ末尾の `(claude)` / `(codex)` で追跡できる。
...
## 手動でイベントを追加する(INBOX方式・推奨)

自動巡回で拾えないイベントを見つけたら、リポジトリ直下の **`INBOX.md`** を
開いて(鉛筆アイコンで編集)、区切り線より下に**イベントページのURLを1行貼って
コミットするだけ**でよい。スマホのGitHubアプリからでも30秒で終わる。

- 翌日の自動更新がURLを開いて日付・場所などを抽出し、カレンダーに載せる
- ブロックされていて自動で開けないサイト(例: NBP・リトアニア中銀)は、URLの
  代わりに自分のブラウザで開いたページの本文(タイトル・日付・場所が入る部分)を
  コピペする
- 取り込み後、区切り線より下は自動で空に戻る
- 既存イベントと重複しても自動で統合されるので気にしなくてよい

## Cloudflare等でブロックされるサイトの手動取込(ファイル方式)

ページ丸ごと(HTML保存など)を渡したい場合はこちら:

- ブロックされるサイト(例: リトアニア中銀)を自分のブラウザで開き、ページを保存
  (またはイベント一覧部分をテキストでコピー)する
- GitHub上で「Add file → Upload files」から `data/manual/` フォルダに
  そのファイル(.html / .txt / .md)をアップロードする
- 翌日の自動更新で内容が読み取られ、カレンダーに取り込まれる
  (処理済みファイルは自動で `data/manual/processed/` に移動する)

## 手動の恒久修正(オーバーライド)

- 自動抽出結果の特定フィールドをずっと固定で直したい場合は、`data/overrides.json` に
  `{"<イベントID>": {"フィールド": 値, ...}}` を書く。次回以降の更新のたびに適用される
  (`data/events.json` はこのファイル経由で書き換えず、表示直前に上書きされる)。
...
- **Codex Cloud 環境の制約**: 検証済みの事実と「試したが駄目だったこと」は
