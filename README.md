# Seminar Radar — ドイツ経済・金融セミナー自動収集カレンダー

毎日1回、AIエージェント(Claude Code Routines または Codex Cloud)がクラウド上で
自動実行され、

1. `sources.yaml` の定点観測リスト(ECB・ブンデスバンク・4大研究所ほか)を巡回
2. web検索で定点リスト外のセミナーも発見
3. 抽出したイベントを `scripts/ingest.py` が検証・重複排除して蓄積
4. `docs/index.html`(カレンダー+一覧)と `docs/calendar.ics` を再生成してプッシュ

する仕組み。GitHub Pages の固定URLをブラウザで開くだけで最新版が見られる。

- **手元PCへのインストール不要**(設定は全てブラウザ)
- **API従量課金なし**: Claude定額課金(Pro/Max)の利用枠内で動く
  (Codex Cloud での併用も検証したが、日次更新には使えないと結論した → §3-b)
- エージェントの役割は「読む・探す・抽出する」だけ。日付検証・重複排除・HTML生成は
  決定論的なPythonコード(`scripts/`)が行うので出力が安定する

---

## 初期セットアップ(ブラウザのみ・所要20分程度)

### 1. GitHubリポジトリを作る
1. github.com にログイン(なければ無料アカウント作成)
2. 右上「+」→「New repository」→ 名前 `seminar-radar`、**Public** → Create
3. リポジトリ画面の「uploading an existing file」(Add file → Upload files)に、
   このフォルダの**中身**(README.md, AGENTS.md, sources.yaml, scripts, data, docs,
   optional)をドラッグ&ドロップ → Commit changes

### 2. GitHub Pages を有効化
Settings → Pages → Source「Deploy from a branch」/ Branch `main`・フォルダ `/docs` → Save。
数分後 `https://<ユーザー名>.github.io/seminar-radar/` が固定URLになる。

### 3. 実行エージェントを設定する

日次更新は Claude Code Routines と Codex Cloud のどちらでも回せる。手順書
(`AGENTS.md`)は両者共通なので、渡すプロンプトは同じ1行でよい。

#### 3-a. Claude Code Routines の場合
1. ブラウザで **claude.ai/code** を開く(定額課金アカウントでログイン)
2. 初回はGitHub連携を求められるので許可し、`seminar-radar` リポジトリへの
   アクセスを与える(全リポジトリではなくこのリポジトリだけに絞ってよい)
3. **claude.ai/code/routines** を開き「New routine」
   - リポジトリ: `seminar-radar`
   - トリガー: Scheduled / 毎日 / 06:30(Europe/Berlin)など好きな時刻
   - プロンプト欄には次の1行だけ書く:

     ```
     リポジトリ直下の AGENTS.md を読み、その指示に従って本日の更新を実行せよ。
     ```

   - ブランチ保護の設定がある場合は「mainへのプッシュを許可」
     (Allow unrestricted branch pushes 相当)を有効にする。
     Routinesは既定で `claude/` 接頭辞のブランチにしかプッシュできないため
4. 保存したら「Run now」(即時実行)で初回を回す

#### 3-b. Codex Cloud は日次更新には使えない(2026-08-17 検証済み)
トークン上限対策として Codex Cloud での併用を検証したが、**日次更新には使えない**と結論した。
エージェントのコンテナからは、web検索が使えず(401)、JS描画ページ用の headless Chromium も
無く、そして **GitHub への書き込みが全面的に遮断されている**(git push・GitHub API とも 403)。
静的ページの巡回だけは正常に動くが、新規発見も成果物の反映もできない。

検証の詳細と「試したが駄目だったこと」の一覧は **`CODEX_NOTES.md`** にある。
Codex で作業する前にこれを読むこと(同じ調査を繰り返さないため)。

なお `.github/workflows/automerge.yml` は `claude/**` と `codex/**` の push で発火し、
`codex/**` 側も動作確認済みなので、将来 Codex から push できるようになれば
自動反映はそのまま機能する。

### 4. 初回実行の確認
1. 実行ログ(claude.ai/code のセッション一覧、またはCodex側の実行ログ)で完了を確認
2. Pages のURLを開き、最下部「巡回ステータス」を見る
3. 失敗しているソースがあれば対処:
   - **URLが古い場合**(特に `verified: false` の IfW/RWI/SUERF/ZEW):
     GitHub上で `sources.yaml` を直接編集(鉛筆アイコン)して正しいURLに直す
   - **ネットワークが遮断されている場合**(403 / host_not_allowed 系のエラー):
     Routineの実行環境は既定で接続先が許可リスト方式のことがある。
     claude.ai/code の環境設定(Environment / Network)で、巡回先ドメイン
     (ecb.europa.eu, bundesbank.de, ifo.de, diw.de, ifw-kiel.de, rwi-essen.de,
     safe-frankfurt.de, imfs-frankfurt.de, hof.uni-frankfurt.de, suerf.org, zew.de)
     を許可リストに追加するか、ネットワーク制限を緩和する

以後は毎日自動で更新される。

---

## 日々の使い方

- 固定URLをブックマークして開くだけ。カレンダー+月別一覧
- フィルタ: テーマ①②③ / 現地・オンライン / 無料のみ / キーワード検索
- 直近4日以内の新検出には **NEW** バッジ
- `docs/calendar.ics` も毎回生成している(将来Outlook等でICS購読が
  可能になった場合に備えたもの)

## 手動でイベントを追加する(INBOX方式・推奨)

自動巡回で拾えないイベントを見つけたら、リポジトリ直下の **`INBOX.md`** を
開いて(鉛筆アイコンで編集)、区切り線より下に**イベントページのURLを1行貼って
コミットするだけ**でよい。スマホのGitHubアプリからでも30秒で終わる。

- 翌日の自動更新がURLを開いて日付・場所などを抽出し、カレンダーに載せる
- ブロックされていて自動で開けないサイト(例: NBP・リトアニア中銀)は、URLの
  代わりに自分のブラウザで開いたページの本文(タイトル・日付・場所が入る部分)を
  コピペする
- 取り込み後、区切り線より下は自動で空に戻る
- 既存イベントと重複しても自動で統合されるので気にしなくてよい

## Cloudflare等でブロックされるサイトの手動取込(ファイル方式)

ページ丸ごと(HTML保存など)を渡したい場合はこちら:

- ブロックされるサイト(例: リトアニア中銀)を自分のブラウザで開き、ページを保存
  (またはイベント一覧部分をテキストでコピー)する
- GitHub上で「Add file → Upload files」から `data/manual/` フォルダに
  そのファイル(.html / .txt / .md)をアップロードする
- 翌日の自動更新で内容が読み取られ、カレンダーに取り込まれる
  (処理済みファイルは自動で `data/manual/processed/` に移動する)

## 手動の恒久修正(オーバーライド)

- 自動抽出結果の特定フィールドをずっと固定で直したい場合は、`data/overrides.json` に
  `{"<イベントID>": {"フィールド": 値, ...}}` を書く。次回以降の更新のたびに適用される
  (`data/events.json` はこのファイル経由で書き換えず、表示直前に上書きされる)。

## メンテナンス(全てブラウザで完結)

- **巡回先の追加・削除・検索観点の変更**: GitHub上で `sources.yaml` を直接編集
- **コードの修正や機能追加**: claude.ai/code(またはCodex)でこのリポジトリを開き、
  日本語で指示すれば編集からコミットまでやってくれる
  (VS CodeのClaude Code拡張でも同じことができるが、ローカルにgitが
  ない環境ではブラウザ版 claude.ai/code を使うのが簡単)
- **エージェントの動作ルール**: 日次更新の手順、ブランチ・コミット方針、
  サブエージェント/下位モデルへの委譲(トークン節約)のルールは
  すべて `AGENTS.md` にある。ルールを変えたいときはこのファイルを編集する
- **Codex Cloud 環境の制約**: 検証済みの事実と「試したが駄目だったこと」は
  `CODEX_NOTES.md` にまとめてある。Codexで作業する前に読むと同じ調査を繰り返さずに済む
- **利用枠**: 日次実行は定額課金の利用枠を消費する。使いすぎが気になる場合は
  実行を週3回などに減らす(`AGENTS.md` A-3 の委譲ルールも消費を抑えるためのもの)
- **トークン消費量の記録**: 巡回1回ごとの消費トークン数は `data/token_usage.json` に
  日次で追記される(`scripts/log_tokens.py` が Claude Code の発話ログから集計する)。
  一覧を見るには `python3 scripts/log_tokens.py --summary`。コミット直前に集計する
  都合上、コミット・プッシュ分は含まれない概算値。この数字はサイトの更新履歴欄にも
  日付ごとに「(◯◯万トークン使用)」として表示される

## 予備手段: GitHub Actions + API従量課金版

Routinesが使えない/やめたい場合のために、同じ処理をGitHub Actions +
Anthropic API(従量課金)で回す版も同梱している。
`optional/daily.yml.disabled` を `.github/workflows/daily.yml` として
コミットし、APIキーをSecrets(`ANTHROPIC_API_KEY`)に登録すれば動く。
本体は `scripts/update_api.py`。詳細はファイル内コメント参照。
