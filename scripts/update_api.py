...
各イベントは次のキーを持つJSONオブジェクトとする:
  ...
  "themes": 次のうち該当するもの全ての配列:
      ...
      "ai"            = AI・デジタル技術の経済・金融・規制への影響(EU AI Act等のAI規制、AIと労働市場・生産性、金融監督・金融サービスにおけるAI、AI産業政策。純粋に技術者向けの開発カンファレンスは除外)
      "china"         = 中国経済・対中経済関係(中国マクロ経済・金融、デリスキング・貿易・投資審査、欧中経済関係)
      "japan"         = 日本経済・日独/日欧経済関係(日本のマクロ経済・金融政策、日独経済フォーラム、日欧経済連携)
      "ukraine"       = ウクライナ関連(ウクライナ経済・金融、復興・戦後再建金融、対ロシア制裁・輸出管理、
                        戦争の経済的影響、NBU等ウクライナ機関の主催)。**そのイベントの中核論点である場合のみ**付ける。
                        安全保障フォーラムのプログラムの一項目として言及されるだけの回には付けない。
                        単独では付けず、必ず分野テーマ(central_bank/real_economy/fin_markets/geopolitics/climate_esg)を併記する
  ...
  "speakers": ページに明記された登壇者の配列(最大8名。基調講演者・高官を優先)。各要素は
      {"name": 氏名(敬称・肩書きなし), "org": 所属機関の略称(organizer_short と同じ規則)または null,
       "role": 役職の短い原文または null}。記載がなければ []。推測で埋めない
  "importance": イベントの重要度。3=機関の年次旗艦会議や大規模国際会議 / 2=中規模の研究会議・
      政策カンファレンス / 1=定例セミナー・小規模講演 / 0=毎週型の定例研究セミナー・
      小規模研究発表(マイクロイベント)
"""

EXTRACTION_RULES = f"""
抽出ルール:
- 上記9テーマのいずれかに該当するイベントのみ抽出する。該当しないものは含めない。
