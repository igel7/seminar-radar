...
- ESG・サステナブルファイナンス・気候・環境関連のイベントは原則すべて収載する。除外するのは毎週型の
  小規模定例研究セミナーのみ(単発の会議・ワークショップ・年次会議は重要度1でも収載)。テーマは
  "climate_esg" を付け、内容に応じて fin_markets(サステナブルファイナンス・気候リスク規制)や
